...
class DeleteCommandTest {
    ...
    fun `user not registered sends warning`() = runBlocking {
        ...
        val telegramId = 72L
        val message = mockMessage("/delete", mockTelegramUser(telegramId))

        coEvery { userService.findByTelegramId(telegramId) } returns null

        val action = DeleteCommand(userService, analytics)
    ...
    fun `registered user gets confirm`() = runBlocking {
        ...
        val analytics = mockk<AnalyticsService>(relaxed = true)
        val telegramId = 73L
        val user = coreUser(id = 51L, telegramId = telegramId)
        val message = mockMessage("/delete", mockTelegramUser(telegramId))
        ...
        coVerify { context.sendHtml(any(), S.deleteDataConfirm, any()) }
    }
}
