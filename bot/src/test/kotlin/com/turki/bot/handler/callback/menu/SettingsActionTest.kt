...
class SettingsActionTest {
    ...
    @Test
    fun `renders settings`() = runBlocking {
        val query = mockCallbackQuery(telegramId = 140L)

        val action = SettingsAction()
        action(context, query, listOf("settings"))

        coVerify { context.editOrSendHtml(query, S.settingsTitle, any()) }
    }
}
