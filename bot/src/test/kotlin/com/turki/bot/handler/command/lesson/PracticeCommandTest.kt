...
class PracticeCommandTest {
    ...
    fun `user not registered sends warning`() = runBlocking {
        ...
        val telegramId = 24L
        val message = mockMessage("/practice", mockTelegramUser(telegramId))

        coEvery { userService.findByTelegramId(telegramId) } returns null

        val action = PracticeCommand(userService, analytics)
    ...
    fun `registered user gets prompt`() = runBlocking {
        ...
        coVerify { context.sendHtml(any(), S.practiceIntro) }
        coVerify { context.sendHtml(any(), S.practicePrompt, any()) }
    }
}
